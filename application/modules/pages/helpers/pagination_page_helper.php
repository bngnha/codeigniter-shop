...
if ( !function_exists('create_pagination_pages'))
{
	function create_pagination_pages($config='')
	{
		$ci =& get_instance();
		$ci->load->library('pagination');
		$ci->load->language('pages/common/pagination');
		...
		if ($pagination != '')
		{
			...
			$pagination .= '<div class="results">'.sprintf(lang('pages.pagination.showing'), $showing_beging, $showing_end, $ci->pagination->total_rows, ceil($ci->pagination->total_rows / $ci->pagination->per_page)).'</div>';
		}
		return $pagination;
	}
}
